...
### Throwaway Flutter spike (`frontend/flutter/`) — DONE, validated, then DISCARDED per spec

Code removed from the repo after validation (see the Disposition row at the
end of this section) — this section is kept as the record of what was built
and proven, not as documentation of files that still exist in the tree.
...
| Disposition | **DISCARDED**, per spec, on explicit request — `frontend/flutter/` and `scripts/fake_echo_server.py` have been removed from the repo. Its purpose was narrow and is done: proving Dart can consume a live, incrementally-arriving WebSocket token stream correctly, which `echo_stream_test.dart`'s real (non-mocked) run confirmed before deletion. This section stays in the doc as the record that the de-risking happened and what it found, even though the code itself is gone - git history (commit `6d55b35`, reverted by the commit after it) still has the actual source if it's ever needed again. Phase 8+'s real Flutter client starts fresh from Part 14.1's spec, not from this code, so nothing of substance was lost by deleting it. |

## Phase 8+ — Real Flutter client (`frontend/flutter/`) — DONE, live-validated against the real backend

Part 14.1: "Phase 8+: Flutter client (second real client). Built after web
client is proven. Shares the same API surface." A fresh `frontend/flutter/`
(the throwaway spike above was fully discarded first, per its own spec, before
this started - nothing here grew out of that code). Same feature set as
`frontend/web/app.js`, validated live earlier this session: onboarding,
WebSocket chat with a static-on-done stage-hints panel, and the four REST CRUD
views (Profile, Decisions, Projects, Providers).

| Item | Status |
|---|---|
| `lib/api_client.dart` | DONE — thin REST wrapper (`http` package) over `/api/v1/*`, one method per endpoint already validated for the web client: `getStatus`, `completeOnboarding`, `getProfile`, `searchDecisions`, `createDecision`, `getProjects`, `createProject`, `activateProject`, `getProviders`, `grantConsent`, `revokeConsent`. Throws `ApiException(statusCode, body)` on any non-2xx; no retries, no caching, no client-side validation beyond what's needed to serialize a request - Part 14.4's "zero intelligence" applies to the Dart client exactly as it does to the JS one. |
| `lib/ws_chat_client.dart` | DONE — mirrors `app.js`'s `connectChat()`/`handleChatEvent()` one for one: same auto-reconnect-on-close behavior (2s delay), same four event types, same Part 14.3 rule that `stage_hint` is buffered and only rendered once `done`/`error` arrives rather than animated live per stage. |
| `lib/onboarding_screen.dart`, `lib/home_shell.dart`, `lib/screens/*.dart` | DONE — onboarding form with the exact same field set and optional/required split as `app.js`'s `submitOnboarding()`; a `ChoiceChip` tab bar (Chat/Profile/Decisions/Projects/Providers) plus a connection-status chip, matching `index.html`'s `#tabs` bar; `IndexedStack` keeps every tab's state alive across switches, the same way the web client's CSS-hidden `<section>`s never leave the DOM. Each of the five views matches its `app.js` counterpart's REST calls and rendered fields one for one. `flutter analyze`: 0 issues. |
| **Real live-backend validation, not just widget tests** | `flutter test`'s widget-test layer can't exercise real network calls, so a second, stronger validation path was used: `tool/validate_live.dart`, a manual (not part of `flutter test`, matching the precedent set by this session's own undocumented live LLM smoke runs against real `llama3.1:8b`) script that imports the actual `lib/api_client.dart` / `lib/ws_chat_client.dart` classes - not a reimplementation - and drives them against a real `uvicorn backend.api.server:app` with a fresh DB. Ran the full flow for real: `GET /status` before/after onboarding, `POST /onboarding/complete`, profile/projects/decisions CRUD including the FTS5 punctuation-tolerant search fixed earlier this session, provider consent grant/revoke, and a full `/ws/chat` round trip against the real `llama3.1:8b` (`"Say hello in exactly one short sentence."` → `"Hello."`, with a real `stage_hint` payload). All checks passed. Kept in the repo (`tool/`, not `lib/` or `test/`) as a reusable manual re-validation script for future backend or client changes, rather than a one-off discarded after use - the real client is production surface, not a throwaway spike, so this has ongoing value the spike's fake-echo tooling didn't. |
| **Real bug found and fixed by this validation, not caught by any existing test** | `GET /api/v1/providers` shipped `is_cloud`/`user_consented`/`revoked` as raw SQLite integers (`0`/`1`) instead of JSON booleans - `api_list_providers()` in `server.py` did `dict(row)` straight from the cursor with no coercion, unlike `api_status()` a few lines above it, which already did `bool(meta["onboarding_complete"])` correctly. The web client's `app.js` never caught this: JavaScript's truthy coercion makes `provider.is_cloud ? 'cloud' : 'local'` work identically for `1`/`0` and `true`/`false`, so the bug was invisible there. Dart's `== true` does not coerce, so the Flutter client's (correctly-written, not defensively-loosened) provider checks caught it immediately. Doubly interesting: `test_provider_consent.py` already had a locked contract - `test_is_cloud_is_native_boolean` - asserting exactly this shape, but only against `config/provider_consent.json`, the static seed file; nothing tested the shape of the live API response the seed data actually becomes after a round trip through SQLite. The two had silently diverged. **Fixed**: `api_list_providers()` now coerces all three fields with `bool()`, matching `api_status()`'s existing convention. **Added `test_providers_api`** to `test_api_server.py` - the missing link, asserting the live `is_cloud`/`user_consented`/`revoked` values via `is True`/`is False` (not `==`, which int `1`/`0` would satisfy against non-native-bool comparisons in a looser language) through consent grant and revoke both, closing the gap between "the seed file is correct" and "the API response is correct" that let this ship unnoticed. |
| Test coverage (backend) | 272 → 273 tests: 1 new (`test_providers_api`), full suite still green after the fix. |
| Manual UI validation — the gap flagged above got closed by the user, not by this session's tooling | This session's own Browser-pane screenshot tool stayed broken (environment-level, not Flutter-specific, same as the spike). But the user ran `flutter run -d web-server` themselves, opened `http://127.0.0.1:8091` in their own real browser, and sent a real screenshot back - which is itself proof the rendering pipeline works (Material widgets, the app's actual error-state UI text, a working `Retry` button), even though what it showed was a failure. |
| **Second real bug, found by that real-browser screenshot, not by `tool/validate_live.dart`** | The screenshot showed `Can't reach PIP - ClientException: Failed to fetch, uri=http://127.0.0.1:8765/api/v1/status`. Root cause: `server.py` had **no CORS middleware at all**. The HTML/JS web client never needed any - it's same-origin, served *by* this same FastAPI app via the `StaticFiles` mount. The Flutter web client is a different origin entirely (`flutter run -d web-server` picks its own port), so a real browser blocks the cross-origin `fetch()` outright. `tool/validate_live.dart` could not have caught this **by construction**, not by bad luck: it runs on the Dart VM (`dart run`, no browser involved), and CORS is a browser-enforced mechanism only - a VM or server-side HTTP client is never subject to it. This is the precise gap between "the real client code works" (proven) and "the real client works from an actual browser tab" (not proven until the user's own screenshot surfaced this). **Fixed**: added `CORSMiddleware` scoped to `http://(localhost\|127.0.0.1)(:\d+)?` - permissive enough for any local dev client on any port, not a bare wildcard, appropriate for an app with no public deployment and no cookie-based auth to leak. **Added `test_cors_allows_local_clients_on_other_ports`** to `test_api_server.py`, using `TestClient` (which sends a real `Origin` header and checks real response headers, unlike the plain `api_*`-function-call tests elsewhere in that file) - the same category of "close the gap between what was tested and what actually runs in a browser" fix as the providers-boolean test above. |
| Test coverage (backend), again | 273 → 274 tests: 1 more new (`test_cors_allows_local_clients_on_other_ports`), full suite still green. |
| macOS platform | `flutter create --platforms=macos .` added the platform scaffold per the user's stated focus ("mac and windows only"). Cannot be built or run on this Windows machine - no way to verify it beyond confirming `flutter analyze`/`flutter test` still pass with the new platform folder present. Untested until opened on an actual Mac. |
| Windows desktop build | Blocked, not fixed: `flutter run -d windows` fails with "Unable to find suitable Visual Studio toolchain" - Visual Studio is installed but missing the "Desktop development with C++" workload (MSVC v142, C++ CMake tools, Windows 10 SDK). This is a large, system-modifying installer change, so it wasn't attempted automatically; the user chose to install it themselves when ready rather than have it done now. Validated via `-d web-server` in the meantime (same app, same code, different target - not a stand-in with reduced coverage, just a different render target while the native toolchain gap gets closed separately). |
| `-d chrome` / `-d edge` also failed, unrelated to the app | Neither `flutter run -d chrome` (no Chrome device registered on this machine, only Edge) nor `-d edge` (browser process failed to launch after 3 tries, likely the same class of GUI-process-launch restriction this session hit with the Browser-pane tool and screenshots all session) worked from this automated environment. `-d web-server` + the user opening the URL in their own real browser was the reliable path, and is exactly what surfaced the CORS bug above - a case where the "worse" tooling path produced the better result. |

## Security review and fix pass

Requested directly: a senior-appsec-style review of the code as it actually
stood (not assumed functionality), checked against explicit architectural
invariants (egress-before-consent, parameterized SQL, transport auth,
provider-metadata trust, plaintext-on-disk exposure, the SQLCipher connection
sequence), then fixed one finding at a time with tests, in severity order.

| # | Finding | Status |
|---|---|---|
| 1 | **CRITICAL - Stage 6 (web search) fired on trigger-keyword match alone, no consent check at all.** `config/provider_consent.json`'s own seed comment claimed "ADR-030: hard_stop gate at Stage 8" for `web_search`; the code never called Stage 8 for it - only for the LLM provider list. Default seed state is `user_consented=0`, so this fired for real on a fresh install before anyone granted anything, and revoking consent had zero effect. | **FIXED** - `pipeline.py` now gates Stage 6 through `stage_08.run(conn, "web_search", requested_scope="web_search_only")` before calling it, fail-closed on `ProviderConsentError`, same pattern already used for LLM providers. 3 new tests in `test_pipeline.py`: blocked by default, fires once consented, blocked again after revoke. |
| 2 | **CRITICAL - zero authentication on the REST API or the WebSocket.** Combined with permissive CORS (which stops cross-origin *reads*, not the request itself), any local process or malicious webpage could blind-call every endpoint. | **FIXED** - new `backend/core/auth.py`: a random token (`secrets.token_hex(32)`) generated on first run, persisted to `data/api_token.txt` (`chmod 0600` best-effort, never logged - not even at startup, only the file's *path* is). `TokenAuthMiddleware` covers every `/api/v1/*` route in one place (not ~20 individual `Depends()`) via a standard `Authorization: Bearer <token>` header; `/ws/chat` checks `?token=` itself (`BaseHTTPMiddleware` doesn't see WS upgrades, and browsers can't set custom headers on a WS connect) and refuses via `close()` *before* `accept()`, so an unauthenticated caller never gets a live connection. A 401 body is always a fixed message, never an echo of what was sent. Both frontend clients (`frontend/web/app.js`, `frontend/flutter/lib/{api_client,ws_chat_client,main,home_shell}.dart`) and `tool/validate_live.dart` send it. New `backend/tests/test_auth.py` (6 tests) + REST/WS auth regression tests in `test_api_server.py`/`test_ws_chat.py`, including non-Bearer-scheme rejection and a body-never-echoes-the-token test. Known, disclosed limit: this stops a *different* process or a malicious webpage, not another process running as the *same* OS user, which can read the same token file - a stronger boundary needs OS-level process identity, out of scope for this fix. |
| 3 | **CRITICAL - `POST /rag/ingest` read any file the process could access, no sandboxing.** Chained with #2 (no auth), this was a full arbitrary-local-file-read-and-exfiltrate primitive (`rag/query` afterward). `SUPPORTED_EXTENSIONS` didn't meaningfully block it - `.txt`/`.json`/`.py` cover most real targets. | **FIXED** - new `vector_store.DOCUMENTS_ROOT` (`data/documents/`) and `_validate_file_path()`: every `file_path` must `Path.resolve()` to somewhere inside it, `..` traversal and symlinks collapsed before the check. 6 new tests across `test_vector_store.py`/`test_stage_05_rag_retrieval.py`/`test_api_server.py`, including the REST-layer regression test (the actual unauthenticated surface an attacker would hit) and a subdirectory-acceptance test so legitimate nested documents still work. |
| 4 | **HIGH - Stage 11 Observer's "local provider only" check (Rule 4) trusted the provider object's own self-reported `is_local`,** unlike every other trust decision in the codebase (Stage 8 included), which anchors `is_cloud` in the operator-controlled `provider_consent` DB table. A future/buggy cloud provider subclass misreporting `is_local=True` would have sent the whole session transcript to it. | **FIXED** - `stage_11_observer.run()` now takes `conn` and requires *both* signals to agree (self-report AND a DB row with `is_cloud=0`), failing closed if the provider_id has no row at all - same posture Stage 8 already uses for unknown providers. `run_session_end()` threads `conn` through. Touched every `FakeProvider`-based test across `test_stage_11_observer.py` (added 1 new negative test), `test_ws_chat.py`, and `test_session_lifecycle.py` to seed a `provider_consent` row for `provider_id="fake"`. |
| 5 | **MEDIUM - `db_key` validated via a bare `assert`,** which `python -O`/`-OO` strips out entirely, silently reopening the f-string-built `PRAGMA key` statement to unchecked input. (A second f-string in `apply_verified_correction` was also flagged per the review's own instruction to report *any* f-string with external input, but was already correctly allowlist-guarded - no change needed there.) | **FIXED** - `profile_store.get_connection()` now does an explicit `if not re.fullmatch(...): raise ValueError(...)`, which survives every optimization level. New `test_get_connection_rejects_non_hex_db_key` in `test_profile_store.py`. |
| 6 | **MEDIUM - sensitive data written to disk in plaintext, outside the SQLCipher boundary.** Two instances: `session_snapshot.json` (topic/decisions/open-problems, regenerated every session-end) and `trace_log.json` (the first 80 chars of every raw user message). | **FULLY FIXED.** `trace_log.json`'s message-content line now logs only `({len(user_message)} chars)`, no content. `session_snapshot.json` is gone entirely - moved into a new `session_snapshot` table in the SQLCipher DB (schema.sql table 20, filling a numbering gap that had sat between 19 and 21) rather than encrypted-in-place, since `conn` was *already* threaded through every call site that used to carry `snapshot_path` (`pipeline.run()`, `stage_11_observer.run_session_end()`, `session_lifecycle.run_observer_now()`/`drain_pending_on_startup()`) - no new crypto dependency needed, and the `snapshot_path` parameter was removable outright rather than kept as a shim. This also structurally retired the real default-argument-binding bug `run_observer_now()`'s docstring used to warn about: there's no path default left to bind wrong at all now. `open_problems`/`last_decisions` are JSON-encoded TEXT columns, same pattern as `decision_candidates_pending.signals_found`. `test_session_snapshot.py` rewritten for the DB-backed version (5 tests, including a fail-open-on-unexpected-DB-error test and a sub-5ms load-time check matching `settings.json`'s `snapshot_load_ms` target); every other affected test file (`test_pipeline.py`, `test_stage_11_observer.py`, `test_session_lifecycle.py`) updated to drop `snapshot_path`/`empty_snapshot_path` entirely - a fresh `conn` already has no `session_snapshot` row, which *is* the "first-ever run" state those fixtures existed to simulate. |
| 7 | **LOW - no WebSocket origin check; message-size limit unverified.** CORS never applies to the WS upgrade at all (HTTP-only in Starlette), so this was a separate gap from #2, not covered by it. | **Origin check FIXED** - `ws_chat()` now rejects a *present but mismatched* `Origin` header (missing Origin is still accepted, for non-browser clients that don't send one at all), defense in depth alongside the token in case it ever leaks. Shares `_ALLOWED_ORIGIN_RE` with `CORSMiddleware` so the two policies can't drift apart. 2 new tests. Message-size limit is still **SUSPECTED/unverified** - depends on the installed `uvicorn`/`websockets` library's own default frame-size limit, not something visible in this codebase; largely subsumed by the auth fix regardless. |

**Live-validated, not just unit-tested**: every fix above was smoke-tested against a real running server before its tests were written - unauthenticated REST/WS calls confirmed rejected (`401`/close code `4401`) and authenticated ones confirmed working via `curl`/a raw Python `websockets` client, then the full `tool/validate_live.dart` re-run end-to-end through the real Flutter client code with a real generated token (9/10 checks passed; the one failure was Ollama being offline on this machine at the time, unrelated to any of the above - the WS connection itself authenticated, connected, and correctly received a real `error` event when the LLM call failed, proving the auth path works on the WS side too).

**Test coverage**: 274 → 297 backend tests across the whole security pass (web-search-consent, `test_auth.py`, REST/WS auth regression including Bearer-scheme and no-echo checks, RAG path-sandboxing, Observer trust verification, db_key validation, WS origin, and the session_snapshot DB migration), full suite green after every individual fix, not just the last one. One flaky test found and fixed along the way (`test_ws_chat_connection_closes_db_on_disconnect`'s disconnect-cleanup poll needed a longer deadline under the suite's current size - reproduced twice under full-suite load, not a one-off, so the ceiling was bumped from 2s to 10s rather than re-tried and ignored).
