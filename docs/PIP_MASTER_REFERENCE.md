...
| 2 | **CRITICAL - zero authentication on the REST API or the WebSocket.** Combined with permissive CORS (which stops cross-origin *reads*, not the request itself), any local process or malicious webpage could blind-call every endpoint. | **FIXED** - new `backend/core/auth.py`: a random token (`secrets.token_hex(32)`) generated on first run, persisted to `data/api_token` (`chmod 0600` best-effort), printed at startup with a ready-to-use URL. `TokenAuthMiddleware` covers every `/api/v1/*` route in one place (not ~20 individual `Depends()`); `/ws/chat` checks `?token=` itself (`BaseHTTPMiddleware` doesn't see WS upgrades) and refuses via `close()` *before* `accept()`, so an unauthenticated caller never gets a live connection. Both frontend clients (`frontend/web/app.js`, `frontend/flutter/lib/{api_client,ws_chat_client,main,home_shell}.dart`) and `tool/validate_live.dart` updated to send it. New `backend/tests/test_auth.py` (6 tests) + REST/WS auth regression tests in `test_api_server.py`/`test_ws_chat.py`. Known, disclosed limit: this stops a *different* process or a malicious webpage, not another process running as the *same* OS user, which can read the same token file - a stronger boundary needs OS-level process identity, out of scope for this fix. |
...
| 6 | **MEDIUM - sensitive data written to disk in plaintext, outside the SQLCipher boundary.** Two instances: `session_snapshot.json` (topic/decisions/open-problems, regenerated every session-end) and `trace_log.json` (the first 80 chars of every raw user message). | **PARTIALLY FIXED.** `trace_log.json`'s message-content line is fixed - it now logs only `({len(user_message)} chars)`, no content, in `pipeline.py`. `session_snapshot.json` is **still open** - closing it properly means either encrypting the file in place (needs a new crypto dependency; nothing in `requirements.txt` provides authenticated symmetric encryption today) or moving its storage into the SQLCipher DB (a wider refactor - `snapshot_path` threads through `pipeline.py`, `stage_11_observer.py`, and `session_lifecycle.py`, and dozens of tests reference it as a file path). Flagged rather than picked unilaterally, given the real dependency/refactor tradeoff either path carries. |
...
**Test coverage**: 274 → 295 backend tests (21 new: 3 web-search-consent, 6 `test_auth.py`, ~6 REST/WS auth regression, 6 RAG path-sandboxing, 1 Observer verification, 1 db_key validation, 2 WS origin), full suite green after every fix, not just the last one.
