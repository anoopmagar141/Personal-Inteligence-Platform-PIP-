...
| **Not built yet** | The broader Response Cache (`core/response_cache.py`), the `/ws/chat` WebSocket endpoint, and all process-lifecycle wiring (idle-timeout Observer triggering, SIGINT handling, `pending_observer.drain()` before Stage 0 at startup) |
