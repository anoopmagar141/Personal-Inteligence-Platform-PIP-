...
| Empirically confirmed, not just documented | The evidence_count=1 discard behavior described below actually happened on the live run: both memory candidates came back `DISCARD` because `profile_age_weeks` for the test profile was past month 2 (requires evidence_count >= 3), and a single-session extraction can only ever produce evidence_count=1. The decision candidate (2 signals, confidence 0.7) correctly auto-logged. |
| **Known scope boundary — not built** | **Cross-session evidence reinforcement** (Part 8.6's "REINFORCEMENT" step: incrementing evidence_count when the same signal recurs across sessions). Every candidate from a single-pass extraction is evidence_count=1 by construction — one transcript can only attest one observation. Without reinforcement, inferred candidates will almost never clear Stage 12's thresholds past Week 1-2, and explicit candidates stop clearing them past Week 3-4 (both require evidence_count >= 2). This needs its own follow-up ticket: extend `stage_12._fetch_existing_state` to also return `evidence_count`, then have the write orchestration increment rather than overwrite it. Flagging this now rather than let Phase 7 look more finished than it is. |
...
| Test coverage | 13 new tests (`test_stage_11_observer.py`, plus 3 in `test_decision_log.py` for `route_observer_decision`), all against a fake provider for speed, plus two live smoke-test runs against real `llama3.1:8b` (not part of the permanent suite — too slow to run every time, ~60-130s per call) |
