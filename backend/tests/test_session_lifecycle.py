...
async def test_an_unreachable_llm_leaves_the_conversation_recoverable(executor_conn):
    ...
    # The mark must not have moved past turns nothing ever read. Read as
    # observed_at rather than through the per-segment high-water mark the
    # original commit used - that column arrived in a different commit on the
    # same branch, and this is the same invariant in the terms main has.
    def _observed_at():
        return conn.execute("SELECT observed_at FROM conversations WHERE id = ?", (cid,)).fetchone()["observed_at"]

    assert await loop.run_in_executor(executor, _observed_at) is None
